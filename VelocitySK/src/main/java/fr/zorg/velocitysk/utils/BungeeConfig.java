...
public enum BungeeConfig {
    ...
    AFFECT_ALL_SERVERS(true, "If true (default), BungeeSK broadcasts and player effects (send, title, kick, send to server, ...) reach every player on the proxy.",
            "Set to false to only affect players whose current server is connected through BungeeSK."),
    ...
    public static void init() {
        try {
            final File file = new File(BungeeSK.getDataDirectory().toFile(), "config.yml");
            YamlFile config;
            if (!file.exists()) {
