...
- **Fixed `make <player> execute proxy command …` doing nothing but chatting on Velocity.** The listener stripped the leading `/` before `spoofChatInput`, which only runs a command when it starts with `/` — so the player just sent the command text as a chat message. It now keeps/adds the slash. BungeeCord was already correct (`dispatchCommand` takes the slashless form).
