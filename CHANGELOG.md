...
- Added a two-server integration test (`scripts/integration-test.sh` + a CI job): boots a real Velocity proxy and a Paper game server, both on the built jar and connected over BungeeSK's socket, and checks the connection-dependent behaviour a single-server boot can't reach — auto-connect, a live request/response round-trip (`network player count` resolving over the socket), keep-alive stability, auto-reconnect after the proxy is dropped and restarted, and a clean shutdown-while-connected. Verified locally (Velocity 3.5.1 + Paper 1.21.11).
